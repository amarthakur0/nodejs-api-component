...
class SequelizeConn {
  ...
  importSchema() {
    try {
      // Table Schema
      const userSchema = reqlib('/components/user/userDbSchema')(SequelizeConn.sequelize),
        userAuthTokenSchema = reqlib('/components/user/userAuthTokenDbSchema')(SequelizeConn.sequelize),
        bookSchema = reqlib('/components/book/bookDbSchema')(SequelizeConn.sequelize);      
      // Set Schema inside Sequelize Connection
      SequelizeConn.schema = {};
      SequelizeConn.schema[userSchema.name] = userSchema;
      SequelizeConn.schema[userAuthTokenSchema.name] = userAuthTokenSchema;
      SequelizeConn.schema[bookSchema.name] = bookSchema;
    }
    catch(e) {
